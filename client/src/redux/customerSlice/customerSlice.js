...
export const logCustomerIn = createAsyncThunk(
  ...
  async (credentials) => {
    const response = await axios.post(
      `http://localhost:8085/user/users//login?email=${credentials.email}&password=${credentials.password}`
    );
...
export const userLogin=(reqObj)=>async dispatch=>{
  ...
  dispatch({type: 'LOADING' , payload:true})
  try{
    const response = await axios.post('http://localhost:8085/user/users/{username}/{password}', reqObj)
    dispatch({type: 'LOADING' , payload:false})
